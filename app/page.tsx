...
async function getHomeEvents(): Promise<EventItem[]> {
  try {
    ...
    const upcomingEvents = await col
      .find({
        ...visibilityFilter,
        date: { $gte: now },
      })
    ...
    const pastEvents =
      upcomingEvents.length < 6
        ? await col
            ...
            .sort({ date: -1 })
            .limit(6 - upcomingEvents.length)
            .toArray()
        : [];

    const eventsFromDb = [...upcomingEvents, ...pastEvents] as MongoEvent[];

    return eventsFromDb.map((event, index) => {
      const eventId = event._id ? String(event._id) : "";

      return {
        id: index + 1,
        title: event.titre || "Événement",
        subtitle: getShortDescription(event.description),
        date: formatDate(event.date),
        location: event.lieu || "",
        image: event.image || "/events/congres.png",
        link: eventId ? `/news/events/${eventId}` : "/news/events",
        category: event.categoryName || "Événement",
      };
    });
  } catch (error) {
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="text-center max-w-3xl mx-auto mb-16">
            ...
            <p className="text-xl text-gray-600 leading-relaxed">
              ...
              accessible et innovant grâce à la recherche scientifique et
      ...
      <section className="relative py-24 bg-gradient-primary text-white overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-6 text-center relative z-10">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <a
              href="/contact"
              className="btn btn-outline border-white text-white hover:bg-white hover:text-primary text-base px-8 py-3"
            >
              Nous Contacter
            </a>
          </div>
